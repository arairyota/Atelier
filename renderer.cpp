...
void CRenderer::SetWorldViewProjection2D(XMFLOAT2 position, XMFLOAT2 rotation, XMFLOAT2 scale, XMMATRIX mtx)
{
	XMMATRIX world;
	world = XMMatrixIdentity();
	
	world *= XMMatrixScaling(scale.x, scale.y, 0.0f);
	//world *= XMMatrixRotationRollPitchYaw(0.0f, rotation.y, rotation.z);
	//world *= XMMatrixRotationY(rotation.x); //XMFLOAT2だから仕方なくyをx
	//world *= XMMatrixRotationZ(rotation.y);
	world *= XMMatrixTranslation(position.x, position.y, 0.0f);
